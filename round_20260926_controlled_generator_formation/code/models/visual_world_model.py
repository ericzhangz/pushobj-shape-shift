...
class VWorldModel(nn.Module):
    ...
    def rollout(self, obs_0, act, *, proprio_transition=None, proprio_feedback=True,
                visual_revision=None, visual_feedback=True, observation_transition=None):
        """
        input:  obs_0 (dict): (b, n, 3, img_size, img_size)
                  act: (b, t+n, action_dim)
        optional: proprio_transition maps normalized (b, 1, 4) proprio and
                  one packed (b, 1, 10) action chunk to next normalized proprio.
                  proprio_feedback=False changes returned proprio only; the
                  recurrent predictor cache stays unmodified. None preserves
                  the original rollout path.
                  visual_revision optionally maps (predicted visual, predicted
                  proprio, revised proprio) to a revised visual token. It requires
                  proprio feedback; visual_feedback=False only changes readout.
                  observation_transition maps the native encoded history and
                  current raw packed action to the next observation token. It
                  replaces predict only inside this same rollout/cache path;
                  it cannot be combined with the other transition callbacks.
        ...
        if observation_transition is not None:
            if (not callable(observation_transition) or self.concat_dim != 1
                    or proprio_transition is not None or visual_revision is not None):
                raise ValueError('observation transition requires one exclusive concat-dim-one path')
        if visual_revision is not None and (proprio_transition is None or not proprio_feedback):
            raise ValueError('visual revision requires the observed proprio feedback path')
        act_0 = act[:, :num_obs_init]
        action = act[:, num_obs_init:] 
        z = self.encode(obs_0, act_0)
        # Optional observed-subsystem revision. The native predictor, action
        # insertion and cache remain authoritative; no second rollout is used.
        if proprio_transition is not None:
            if (self.concat_dim != 1 or self.num_proprio_repeat != 1
                    or z.shape[2] != 1 or obs_0['proprio'].shape[-1] != 4):
                raise ValueError('proprio transition requires the native PushObj layout')
            proprio_state = obs_0['proprio'][:, -1:]
            proprio_predictions = []
            visual_predictions = []

        def revise_proprio(prediction, action_index):
            nonlocal proprio_state
            proprio_state = proprio_transition(proprio_state, act[:, action_index:action_index+1])
            encoded_proprio = self.encode_proprio(proprio_state).unsqueeze(2)
            if (encoded_proprio.shape != prediction[..., -(self.proprio_dim+self.action_dim):-self.action_dim].shape
                    or not torch.isfinite(encoded_proprio).all()):
                raise ValueError('invalid revised proprio token')
            proprio_predictions.append(encoded_proprio)
            visual = prediction[..., :-(self.proprio_dim+self.action_dim)]
            if visual_revision is not None:
                revised_visual = visual_revision(
                    visual, prediction[..., -(self.proprio_dim+self.action_dim):-self.action_dim],
                    encoded_proprio)
                if revised_visual.shape != visual.shape or not torch.isfinite(revised_visual).all():
                    raise ValueError('invalid revised visual token')
                visual_predictions.append(revised_visual)
                if visual_feedback:
                    visual = revised_visual
            if not proprio_feedback:
                return prediction
            return torch.cat((visual,
                              encoded_proprio, prediction[..., -self.action_dim:]), dim=-1)

        def next_observation(action_index):
            history = z[:, -self.num_hist:]
            if observation_transition is None:
                return self.predict(history)[:, -1:]
            observation = observation_transition(history, act[:, action_index:action_index+1])
            expected = history[:, -1:, ..., :-self.action_dim]
            if (not torch.is_tensor(observation) or observation.shape != expected.shape
                    or observation.dtype != expected.dtype or observation.device != expected.device
                    or not torch.isfinite(observation).all()):
                raise ValueError('invalid shared observation transition output')
            # The native insertion below remains authoritative for future actions.
            return torch.cat((observation, history[:, -1:, ..., -self.action_dim:]), dim=-1)

        t = 0
        inc = 1
        while t < action.shape[1]:
            z_new = next_observation(num_obs_init - 1 + t)
            if proprio_transition is not None:
                z_new = revise_proprio(z_new, num_obs_init - 1 + t)
            z_new = self.replace_actions_from_z(z_new, action[:, t : t + inc, :])
            z = torch.cat([z, z_new], dim=1)
            t += inc

        z_new = next_observation(act.shape[1] - 1)
        if proprio_transition is not None:
            z_new = revise_proprio(z_new, act.shape[1] - 1)
        z = torch.cat([z, z_new], dim=1)
        if proprio_transition is not None and not proprio_feedback:
            # Readout-only control: all recurrent visual predictions above used
            # the unmodified cache. Return the same proprio path as feedback.
            proprio_path = torch.cat((z[:, :num_obs_init, :, -(self.proprio_dim+self.action_dim):-self.action_dim],
                                      *proprio_predictions), dim=1)
            z = torch.cat((z[..., :-(self.proprio_dim+self.action_dim)],
                           proprio_path, z[..., -self.action_dim:]), dim=-1)
        if visual_revision is not None and not visual_feedback:
            visual_path = torch.cat((z[:, :num_obs_init, :, :-(self.proprio_dim+self.action_dim)],
                                     *visual_predictions), dim=1)
            z = torch.cat((visual_path, z[..., -(self.proprio_dim+self.action_dim):]), dim=-1)
